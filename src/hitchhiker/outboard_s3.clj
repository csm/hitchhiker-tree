...
(defn- launch-outboard-processer!
  ...
  (let [^LinkedBlockingQueue q (:update-queue conn)
        ...
        flush-tree #(when-not (hhs3/get-root-key @tree-atom)
                      (swap! tree-atom
                             (fn [tree]
                               (:tree (core/flush-tree
                                        tree
                                        (hhs3/->S3Backend bucket)))))
                      (let [new-root (hhs3/get-root-key @tree-atom)]
                        (hhs3/write-object bucket "ROOT"
                                           (nippy/freeze {:key new-root}))))]
    (reset! (:thread conn)
            (doto (Thread.
                    (fn* []
...
(defn create
  ...
  (let [conn (->OutboardConnection (LinkedBlockingQueue.) (atom (core/b-tree (core/->Config 30 600 870))) (atom :running) (atom nil) new-name)]
    (launch-outboard-processer! conn new-name bucket-name)
    (swap! connection-registry assoc new-name conn)
    conn))

; destroy is TODO, kinda don't want that nuke available
; anyway this would delete all objects, then delete the bucket
...
;;   [bucket]
;;   (when-not (string? bucket)
;;     (throw (ex-info "destroy takes the name of an outboard" {:name name})))
;;   (when (contains? @connection-registry bucket)
...
(defn open
  ...
  [bucket]
  (or (get @connection-registry bucket)
      (if-let [root-key (with-open [in (:input-stream (s3/get-object bucket "ROOT"))]
                          (nippy/thaw (ByteStreams/toByteArray in)))]
        (let [conn (->OutboardConnection
                     (LinkedBlockingQueue.)
                     (atom (hhs3/create-tree-from-root-key root-key))
                     (atom :running)
                     (atom nil)
                     name)]
          (launch-outboard-processer! conn bucket)
          (swap! connection-registry assoc bucket conn)
          conn)
        (throw (ex-info (str "Didn't find root-addr at " bucket) {})))))
